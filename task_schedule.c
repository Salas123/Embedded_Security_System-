...
enum SM1_States { SM1_AwaitMotion, SM1_MotionDetected};
...
int SMTick1(int state) {
	...
	// Local Variables
	unsigned char pirMotionRead = PINC & 0x10;

	//State machine transitions
	switch (state)
	{
		case SM1_AwaitMotion:
		{ 	
			if (pirMotionRead == 0x01) 	// Wait for button press
				state = SM1_MotionDetected;
			else	
				state = SM1_AwaitMotion;

			break;
		}

		case SM1_MotionDetected:
		{
			state = SM1_AwaitMotion;
			break;
		}
...
unsigned long int SMTick3_calc = 2000;
